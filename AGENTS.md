...
- **README ↔ docs sync:** The `README.md` and the documentation website (built from `docs/` via zensical, deployed at <https://el1t3mult1pl3x0r.github.io/rainbowminer-api-client/>) cover the same content for end users. Any change to `README.md` that adds, removes, or modifies user-facing information (features, installation, quick start, examples, endpoint tables, etc.) must be mirrored in the corresponding `docs/` page, and vice versa. Do not update one without the other.
